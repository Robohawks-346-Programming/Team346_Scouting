...
def sync_to_github(success_col) -> None:
    ...
    repo.update_file(
        ...
        str(file_json_data).replace("'", '"').replace("True", "1").replace("False", "0"),
        contents.sha,
    )
    ...
    repo.update_file(
        contents.path,
        f'updated data @ {datetime.datetime.now().strftime("%m/%d/%Y, %H:%M:%S")}',
        str(qualitative_json_data).replace("'", '"').replace("True", "1").replace("False", "0"),
        contents.sha,
    )
...
def display_data() -> None:
    ...
    data_display_col.write("### 📝 Pit Scouting Data Editor")
